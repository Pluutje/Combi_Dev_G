...
class Therapy(private val persistenceLayer: PersistenceLayer){
    var laagCarbTime = false
    var gemCarbTime = false
    var hoogCarbTime = false
    var BstTime = false
    ...
    fun updateStatesBasedOnTherapyEvents() {
        ...
        if (!stopTime) {
            laagCarbTime = findActivelaag_CarbEvents(System.currentTimeMillis()).blockingGet()
            gemCarbTime = findActivegem_CarbEvents(System.currentTimeMillis()).blockingGet()
            hoogCarbTime = findActivehoog_CarbEvents(System.currentTimeMillis()).blockingGet()
            BstTime = findBst_Events(System.currentTimeMillis()).blockingGet()
            ...
            resetAllStates()
            clearActiveEvent("LKh")
            clearActiveEvent("MKh")
            clearActiveEvent("HKh")
            clearActiveEvent("Boost")
    ...
    fun findActivelaag_CarbEvents(timestamp: Long): Single<Boolean> {
        ...
        return persistenceLayer.getTherapyEventDataFromTime(fromTime, true)
            .map { events ->
                events.filter { it.type == TE.Type.NOTE } // Filtrer les événements par type
                    .any { event ->
                        event.note?.contains("LKh", ignoreCase = true) == true &&
                            System.currentTimeMillis() <= (event.timestamp + event.duration)
                    }
            }
    }
    fun findActivegem_CarbEvents(timestamp: Long): Single<Boolean> {
        val fromTime = timestamp - TimeUnit.DAYS.toMillis(1) // les dernières 24 heures
        // Utiliser la méthode getTherapyEventDataFromTime avec le timestamp et l'ordre de tri
        return persistenceLayer.getTherapyEventDataFromTime(fromTime, true)
            .map { events ->
                events.filter { it.type == TE.Type.NOTE } // Filtrer les événements par type
                    .any { event ->
                        event.note?.contains("MKh", ignoreCase = true) == true &&
                            System.currentTimeMillis() <= (event.timestamp + event.duration)
                    }
            }
    }
    fun findActivehoog_CarbEvents(timestamp: Long): Single<Boolean> {
        val fromTime = timestamp - TimeUnit.DAYS.toMillis(1) // les dernières 24 heures
        // Utiliser la méthode getTherapyEventDataFromTime avec le timestamp et l'ordre de tri
        return persistenceLayer.getTherapyEventDataFromTime(fromTime, true)
            .map { events ->
                events.filter { it.type == TE.Type.NOTE } // Filtrer les événements par type
                    .any { event ->
                        event.note?.contains("HKh", ignoreCase = true) == true &&
                            System.currentTimeMillis() <= (event.timestamp + event.duration)
                    }
            }
    }
    fun findBst_Events(timestamp: Long): Single<Boolean> {
        val fromTime = timestamp - TimeUnit.DAYS.toMillis(1) // les dernières 24 heures
        // Utiliser la méthode getTherapyEventDataFromTime avec le timestamp et l'ordre de tri
        return persistenceLayer.getTherapyEventDataFromTime(fromTime, true)
            .map { events ->
                events.filter { it.type == TE.Type.NOTE } // Filtrer les événements par type
                    .any { event ->
                        event.note?.contains("Boost", ignoreCase = true) == true &&
                            System.currentTimeMillis() <= (event.timestamp + event.duration)
                    }
            }
    }
    ...
    fun findActivestopEvents(timestamp: Long): Single<Boolean> {
        val fromTime = timestamp - TimeUnit.DAYS.toMillis(1) // les dernières 24 heures
        return persistenceLayer.getTherapyEventDataFromTime(fromTime, true)
            .map { events ->
                events.filter { it.type == TE.Type.NOTE }
    ...
    fun getTimeElapsedSinceLastEvent(keyword: String): Long {
        val fromTime = System.currentTimeMillis() - TimeUnit.MINUTES.toMillis(180)
